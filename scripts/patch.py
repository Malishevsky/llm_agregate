...
def _get_env_path(logger: Logger, root_folder_path: Path, base_commands: list[str]) -> Path:
    ...
    if env_path is not None:
        ...
        if root_folder_path.joinpath(_PIPFILE).exists():
            ...
            sys.exit(1)
        try:
            env_path_output = run(command, capture_output=True, text=True, check=True, cwd=root_folder_path)  # noqa: S603, DUO116
...
# TODO: Пересмотреть логику проверки, предпочтительно вернуться к старому варианту
def _check_python_runner(python_runner: str, package_manager: str, root_folder_path: Path) -> bool:
    try:
        run([python_runner, '--version'], capture_output=True, text=True, check=True, cwd=root_folder_path)  # noqa: S603, DUO116
        return True
    except BaseException:  # noqa: PIE786, WPS424
        return False
...
def _get_package_commit_id(package_name: str, requirements: list[str], logger: Logger) -> str | None:
    ...
    commit_id = pkg_freeze_info.split('@')[-1]
...
def _main(logger: Logger) -> int:  # noqa: WPS213
    ...
    base_commands = [python_runner, '-m', package_manager]
